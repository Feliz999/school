...
<div class="container">
    <div class="row">
        <div class="col text-center shadow-lg p-3 mb-5 bg-body rounded">
            ...
            <hr>
            <form action="{{route('typePeople.store')}}" method="post" class="form">
                @csrf
        ...
        <div class="col-md-9 text-center shadow-lg p-3 mb-5 bg-body rounded">
            ...
            @if(isset($listTypePeople))
                <table class="table table-striped table-hover">
                    ...
                    @foreach($listTypePeople as $tp)
                    <tr>
                        <td>{{$loop->iteration}}</td>
                        <td>{{$tp->name}}</td>
                        <td>{{$tp->description}}</td>
                        <td>
                            <a href="" data-bs-toggle="modal" data-bs-target="#editarTipo-{{$tp->id}}"><i data-fa-symbol="edit" class="fa-solid fa-pencil fa-fw"></i></a>
                            <a href="" data-bs-toggle="modal" data-bs-target="#eliminarTipo-{{$tp->id}}"><i data-fa-symbol="delete" class="fa-solid fa-trash fa-fw" style="color:red"></i></a>
                        </td>
                    </tr>
                        @include('type_people.editar')
                        @include('type_people.delete')
                    @endforeach
                </table>
                {{ $listTypePeople->links() }}
            @else
